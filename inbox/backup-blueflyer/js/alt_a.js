...
const ALT_TEXT_CONFIG = {
    ...
    apiKey: "test-key-local-dev-2024",
        ...
        try {
            ...
            const requestBody = {
                ...
                messages: [
                    {
                        role: "system",
                        content: ALT_TEXT_CONFIG.systemPrompt
                    },
                    {
                        role: "user",
                        content: [
            ...
            const response = await fetch('https://api.assisted.space/camina/chat', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-API-Key': ALT_TEXT_CONFIG.apiKey,
                    // No need for anthropic-version or beta headers since we handle that server-side
                },
                                ...
                                for (const line of lines) {
                                    ...
                                    if (line.startsWith('data: ')) {
                                        try {
                                            ...
                                            if (jsonData.type === 'message_delta' && jsonData.delta?.text) {
                                                console.log('Parsed content:', jsonData.delta.text);
