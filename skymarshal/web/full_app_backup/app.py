...
from flask import Flask, render_template, request, jsonify, session, redirect, url_for, Response, stream_with_context, send_from_directory, current_app
import secrets
import threading
import queue
import time
...
def login():
    if request.method == 'POST':
        ...
        try:
            # Check if password looks like regular password before attempting auth
            used_regular_password = is_likely_regular_password(password)
            ...
            if auth_manager.authenticate_client(normalized_handle, password):
                ...
                # Return success with warning if regular password was used
                response = {'success': True, 'redirect': url_for('setup')}
                if used_regular_password:
                    response['warning'] = '⚠️ Warning: You appear to be using your regular Bluesky password. For security, we recommend using an app password from Settings > Privacy & Security > App Passwords.'
                ...
                if used_regular_password:
                    return jsonify({
                        'success': False,
                        'error': 'Authentication failed. You appear to be using your regular password. Please use an app password from Bluesky settings for better security.',
...
def user_profile():
    ...
    handle = session.get('user_handle')
    if not handle:
        return jsonify({'success': False, 'error': 'Missing session handle'}), 401

    if not auth_manager.is_authenticated():
        return jsonify({'success': False, 'error': 'Session expired'}), 401

    profile_data, method_used = None, None

    # Method 1: the modern app.bsky.actor.getProfile endpoint
    try:
        ...
        current_app.logger.debug("get_profile RPC failed: %s", exc)

    # Method 2: legacy convenience helper
    if profile_data is None:
        try:
            profile_data = auth_manager.client.get_profile(handle)
            method_used = 'client.get_profile'
        except Exception as exc:
            current_app.logger.warning("Legacy profile fetch failed: %s", exc)

    if profile_data is None:
        return _user_profile_fallback(handle, 'Profile lookup failed')

    def _extract(attr_name: str, *aliases):
        for name in (attr_name, *aliases):
            value = getattr(profile_data, name, None)
            if value is not None:
                return value
        return None

    payload = {
        'handle': _extract('handle') or handle,
        'displayName': _extract('display_name', 'displayName') or '',
        'description': _extract('description') or '',
        'avatar': _extract('avatar') or '/static/images/default-avatar.svg',
        'banner': _extract('banner') or '',
        'followersCount': _extract('followers_count', 'followersCount') or 0,
        'followsCount': _extract('follows_count', 'followsCount') or 0,
        'postsCount': _extract('posts_count', 'postsCount') or 0,
    }

    created_at = _extract('created_at', 'createdAt')
    if created_at and hasattr(created_at, 'isoformat'):
        payload['createdAt'] = created_at.isoformat()
    else:
        payload['createdAt'] = created_at

    return jsonify({'success': True, 'method_used': method_used, 'profile': payload})


def _user_profile_fallback(handle: str, error: str):
    current_app.logger.warning("Profile fallback for %s: %s", handle, error)
    return jsonify({
        'success': True,
        'profile': {
            'handle': handle,
...
@login_required
def nuke():
    """Nuclear delete page - delete ALL content with multiple confirmations"""
    handle = session['user_handle']
    json_path = get_json_path()
    ...
    try:
        ...
        stats = {
            'total_posts': len(posts),
            'total_replies': len(replies),
            'total_likes': len(like_items),
            'total_reposts': len(repost_items),
            'total_items': total_items
        }
        ...
        print(f"Error loading data for nuclear delete: {e}")
        return redirect(url_for('dashboard'))

@app.route('/nuke', methods=['POST'])
@login_required
def nuke_execute():
    """Execute nuclear delete with confirmations"""
    data = request.get_json()
    confirmation_phrase = data.get('confirmation_phrase', '')
    final_confirmation = data.get('final_confirmation', False)
    
    handle = session['user_handle']
    expected_phrase = f"DELETE ALL {handle}"
    
    # Validate confirmations
    if confirmation_phrase != expected_phrase:
        return jsonify({'success': False, 'error': 'Confirmation phrase does not match'}), 400
    
    if not final_confirmation:
        return jsonify({'success': False, 'error': 'Final confirmation checkbox must be checked'}), 400
    
    # Get all content to delete
    json_path = get_json_path()
    if not json_path:
    ...
    try:
        ...
        if not all_items:
            return jsonify({'success': False, 'error': 'No content found to delete'}), 400
        
        # Delete all items using the deletion manager
        deletion_manager = DeletionManager(auth_manager=auth_manager, settings=settings)
...
@app.route('/analytics/followers', methods=['POST'])
@login_required
def analytics_followers():
    """Follower analysis endpoint"""
    try:
        data = request.get_json()
        ...
        follower_analyzer = FollowerAnalyzer(auth_manager)
        
        if action == 'analyze':
            max_followers = data.get('max_followers', 100)
            results = asyncio.run(follower_analyzer.run_complete_analysis(
                target_username=session.get('user_handle'),
...
@app.route('/analytics/posts', methods=['POST'])
@login_required
def analytics_posts():
    ...
    try:
        ...
        post_analyzer = PostAnalyzer(auth_manager)
        
        if action == 'analyze':
            max_posts = data.get('max_posts', 100)
            results = asyncio.run(post_analyzer.run_complete_analysis(
                author_handle=session.get('user_handle'),
                max_posts=max_posts
            ))
            return jsonify({'success': True, 'results': results})
        else:
...
@app.route('/analytics/content', methods=['POST'])
@login_required
def analytics_content():
    ...
    try:
        ...
        content_analyzer = ContentAnalyzer(auth_manager)
        ...
        if action == 'analyze':
            ...
            data_manager = DataManager(auth_manager, settings, skymarshal_dir, backups_dir, json_dir)
            json_path = get_json_path()
            
            if not json_path:
                return jsonify({'success': False, 'error': 'No data available for analysis'}), 400
            
            posts = data_manager.load_exported_data(Path(json_path))
            post_data = [{'text': post.text, 'created_at': post.created_at} for post in posts if post.text]
            
            analysis_types = data.get('analysis_types', ['vibe', 'summary', 'sentiment', 'categorize'])
            results = asyncio.run(content_analyzer.run_complete_analysis(post_data, analysis_types))
            return jsonify({'success': True, 'results': results})
        else:
            return jsonify({'success': False, 'error': 'Invalid action'}), 400
...
def cleanup_dashboard():
    ...
    return render_template('cleanup_dashboard.html')

@app.route('/cleanup/following', methods=['POST'])
@login_required
def cleanup_following():
    """Following cleanup endpoint"""
    try:
        data = request.get_json()
        action = data.get('action', 'analyze')
        ...
        following_cleaner = FollowingCleaner(auth_manager)
        
        if action == 'analyze':
            max_following = data.get('max_following', 200)
            results = asyncio.run(following_cleaner.run_complete_analysis(max_following=max_following))
            return jsonify({'success': True, 'results': results})
        elif action == 'unfollow':
            interactive = data.get('interactive', False)
            results = asyncio.run(following_cleaner.run_complete_analysis(interactive=interactive))
            return jsonify({'success': True, 'results': results})
        else:
            return jsonify({'success': False, 'error': 'Invalid action'}), 400
...
def cleanup_posts():
    ...
    try:
        ...
        post_importer = PostImporter(auth_manager)
        
        if action == 'import':
            max_posts = data.get('max_posts', 100)
            results = asyncio.run(post_importer.run_complete_import(
                author_handle=session.get('user_handle'),
                max_posts=max_posts
            ))
            return jsonify({'success': True, 'results': results})
...
@app.route('/cleanup/health', methods=['POST'])
@login_required
def cleanup_health():
    ...
    try:
        ...
        if action == 'check' or action == 'quick_check':
            # Run comprehensive health check
            following_cleaner = FollowingCleaner(auth_manager)
            post_analyzer = PostAnalyzer(auth_manager)
            
            # Get following analysis
            following_results = asyncio.run(following_cleaner.run_complete_analysis(max_following=100))
            
            # Get post analysis
            post_results = asyncio.run(post_analyzer.run_complete_analysis(
                author_handle=session.get('user_handle'),
                max_posts=50
            ))
            ...
            if following_results.get('high_risk_count', 0) > 0:
                health_score -= following_results['high_risk_count'] * 2
                recommendations.append(f"Consider unfollowing {following_results['high_risk_count']} high-risk accounts")
            
            if following_results.get('low_quality_count', 0) > 0:
                health_score -= following_results['low_quality_count']
                recommendations.append(f"Review {following_results['low_quality_count']} low-quality accounts")
            
            if post_results.get('analysis', {}).get('avg_engagement', 0) < 5:
                health_score -= 10
                recommendations.append("Consider improving post engagement")
            
            health_score = max(0, min(100, health_score))
            
            results = {
                'health_score': health_score,
                'following_quality': 'Good' if following_results.get('high_risk_count', 0) < 5 else 'Needs Attention',
                'engagement_rate': f"{post_results.get('analysis', {}).get('avg_engagement', 0):.1f}",
                'recommendations': recommendations,
                'following_analysis': following_results,
...
@login_required
def api_stats():
    """Get account statistics for unified dashboard"""
    try:
        auth_manager = get_auth_manager()
        if not auth_manager:
            return jsonify({'success': False, 'error': 'Not authenticated'}), 401
        
        # Get basic stats from session or calculate
        json_path = get_json_path()
        if json_path:
            from skymarshal.data_manager import DataManager
            from skymarshal.settings import SettingsManager
            ...
            total_posts = len([item for item in items if item.content_type == ContentType.POST])
            total_likes = sum(item.like_count for item in items)
            total_reposts = sum(item.repost_count for item in items)
            total_replies = sum(item.reply_count for item in items)
            
            # Simple health score calculation
            health_score = 85  # Default score
            if total_posts > 0:
                avg_engagement = (total_likes + total_reposts + total_replies) / total_posts
            ...
            stats = {
                ...
                'total_followers': session.get('total_followers', 0),
                'total_following': session.get('total_following', 0),
                'health_score': health_score,
...
@login_required
def api_activity():
    """Get recent activity for unified dashboard"""
    try:
        ...
        activities = [
            {
                'icon': 'chart-line',
                'color': 'primary',
                'description': 'Follower analysis completed',
                'timestamp': '2 hours ago'
            },
            {
                'icon': 'broom',
                'color': 'warning',
                'description': 'Following cleanup completed',
                'timestamp': '1 day ago'
            },
            {
                'icon': 'download',
